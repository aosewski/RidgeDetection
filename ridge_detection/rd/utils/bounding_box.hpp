...
/**
 * @brief      Represents bounding box of multidimensional points set.
 *
 * @tparam     T     Data type.
 */
template <typename T>
struct BoundingBox
{
	/// Pointer to array of respective dimensions bounds
	T *bbox;
	size_t dim;
	/// Pointer to array of distances between min and max for respective dimensions
	T *dist;
	...
	/**
	 * @brief      Counts how many spheres fits inside region described by this bounding box
	 *
	 * @param[in]  radius  Sphere radius.
	 *
	 * @return     the number of spheres that fits in this bounding box region.
	 */
	...
	inline bool overlap(BoundingBox<T> const &other)
	{
		...
		for (size_t d = 0; d < dim; ++d)
		{
			...
			*/
			// does two segments overlap? 
			result = false;
			// min(this)  min(other)    max(this)
			//   |----------|=========------|
			result = result || (other.min(d) >= min(d) && other.min(d) <= max(d));

			// min(this)  max(other)	max(this)
			//   |----========|--------------|
			result = result || (other.max(d) >= min(d) && other.max(d) <= max(d));

			// min(other)  min(this)	max(this)    max(other)
			//   |------------|============|------------|
			result = result || (other.min(d) <= min(d) && other.max(d) >= max(d));
			if (!result)
				return false;
		}
