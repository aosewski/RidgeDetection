...
/**
 * @brief      Class providing graph drawing tools.
 *
 *             Uses gnuplot interface to draw graphs. User can have precise control on performed
 *             commands i.e. using addPlotCmd, or use predefined configurations.
 *
 * @tparam     T     Data type.
 */
template <typename T>
...
public:
	...
	/**
	 * @brief      Defines supported basic figures.
	 */
	enum FIGURE
	{
		POINTS,
		...
		currentGraphDim(0)
	{
		...
		sendCmd("set key off");	 // turn off key
		sendCmd("set size ratio -1");		// equal scale on all axes
	...
	{
		sendCmd(std::string("set style line ") + 
			std::to_string(++stylesCnt) + std::string(" ") + style);
		return stylesCnt;
	}
	...
	/**
	 * @brief      Begin graph drawing command.
	 *
	 * @param      filename  Outpu graph filename
	 * @param[in]  dim       Data dimensions. Default two.
	 */
	...
	/**
	 * @brief      End graph drawing command and draw graph.
	 */
	void endGraph()
	{
