...
private:
        ...
        const int               t)
    {
        ...
        for (int k = 0; k < TILE_W; ++k)
        {
            ...
            for (int r = 0; r < ITEMS_PER_THREAD; ++r)
            {
                // we ommit checking condition (threadIdx.y * ITEMS_PER_THREAD + r < height)
                // becaouse we have already zeroed that part of As, thus it won't affect 
                // overall result
                T diff = tileLoader.smem.As[tileRowOffset + r][k] - atVal;
                sqrDist[r] += diff * diff;
            }
        ...
        const int               t)
    {
        ...
        for (int k = 0; k < TILE_W; ++k)
        {
            ...
            // but we alreade have zeroed this part, and thus we can avoid divergence
            T atVal = tileLoader.smem.AsT[k][threadIdx.x];
            #pragma unroll
            for (int r = 0; r < ITEMS_PER_THREAD; ++r)
            {
