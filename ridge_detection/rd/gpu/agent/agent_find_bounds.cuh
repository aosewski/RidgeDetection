...
template <
    ...
    cub::BlockReduceAlgorithm   _REDUCE_ALGORITHM,              ///< The BlockReduce algorithm to use
...
struct AgentFindBoundsPolicy
{
    ...
    static const cub::BlockReduceAlgorithm   REDUCE_ALGORITHM    = _REDUCE_ALGORITHM;        ///< The BlockReduce algorithm to use
...
public:
    ...
    __device__ __forceinline__ AgentFindBoundsBase(
        ...
        SampleT const *     d_samples,                          ///< Input data to reduce
        SampleT*            (&d_outputBboxMin)[DIM],          ///< Reference to final output bounds
        SampleT*            (&d_outputBboxMax)[DIM])          ///< Reference to final output bounds
    :
...
/**
 * @brief      Defines first phase of (two-phase) found bounds algorithm.
 *
 *             In first pass each block of threads reads and process tiles of input data. It finds both max and min
 *             bounds for each dimension and stores it's (block) private results to the output temporaray storage.
 *
 * @tparam     AgentFindBoundsPolicyT  Algorithm tuning parameters
...
private:
    ...
    __device__ __forceinline__ void consumeTiles(
        OffsetT     numPoints)              ///< The number of multi-dimensional points to consume
    {
...
public:
     ...
     */
    __device__ __forceinline__ void consumeTiles(
        OffsetT     num_points,     ///< The number of multi-dimensional points to consume
        OffsetT     stride)         ///< The number of samples between starts of consecutive points's coordinates 
    {
        consumeTiles(num_points);
    }
};
 ...
 *
 *             In this pass there are as many blocks running as point's dimensions times two. Evenly indexed blocks
 *             perform max reduction of block-private results from algorithm first pass for subsequent dimensions. While
 *             odd indexed perform min reduction.
 *
 * @tparam     AgentFindBoundsPolicyT  Algorithm tuning parameters
...
public:
    ...
    __device__ __forceinline__ AgentFindBoundsSecondPass(
        TempStorage &                               tempStorage,              ///< Reference to tempStorage
        cub::ArrayWrapper<SampleT *, DIM> const &   d_inMin,                  ///< Reference to input data to reduce. One pointer per region (DIM).
        cub::ArrayWrapper<SampleT *, DIM> const &   d_inMax,                  ///< Reference to input data to reduce. One pointer per region (DIM).
        SampleT *                                   d_outputBboxMin,          ///< pointer to final output bounds
        SampleT *                                   d_outputBboxMax)          ///< pointer to final output bounds
    :
        tempStorage(tempStorage.Alias())
    {       
        // even indexed blocks perform max reduction
     ...
     */
    __device__ __forceinline__ void consumeTiles(
        OffsetT     numPoints)              ///< The number of samples to consume
    {
