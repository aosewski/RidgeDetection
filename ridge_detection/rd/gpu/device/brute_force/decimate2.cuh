 ...
 * @param[in|out] ns    Pointer to device global memory, containing number of point in @p S set.
 * @param[in]     r     Algorithm parameter. The radius at which we search neighbours.
 *
 * @tparam        T                Data type
 * @tparam        BLOCK_SIZE       Number of threads in block.
 * @tparam        VALS_PER_THREAD  Number of values to process by individual thread.
