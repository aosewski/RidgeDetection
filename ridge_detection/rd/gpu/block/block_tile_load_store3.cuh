/*-------------------------------------------------------------------------*//**
 * @file block_tile_load_store3.cuh
...
class BlockTileLoad<BlockTileLoadPolicyT, DIM, rd::COL_MAJOR, IO_BACKEND, SampleT, OffsetT>
{
    ...
    struct LoadInternal<IO_BACKEND_TROVE, DUMMY>
    {
        ...
        /// Helper structure for templated copy samples from TroveArray to C array 
        struct CopyCol2RowsIterateCols
        {
            ...
            static __device__ __forceinline__ void impl(
                ...
                TroveArray const &  ary)
            {
                samples[P::value][D::value] = trove::get<P::value>(ary);
            }
        };
        ...
        struct LoadFullTile2Row
        {
            ...
            {
                TroveArray tArray;
                tArray = trove::load_array_warp_contiguous<TroveArray::size>(d_inputSamples + D::value * stride, threadIdx.x);
                StaticFor<0, POINTS_PER_THREAD, CopyCol2RowsIterateCols>::impl(dIdx, samples, tArray);
            }
        };  
        ...
        static __device__ __forceinline__ void loadTile2Row(
            ...
            cub::Int2Type<true>             canVectorize)
        {
            StaticFor<0, DIM, LoadFullTile2Row>::impl(d_inputSamples, samples, stride);
        }
            ...
            OffsetT                         stride)
        {
            ...
            for (int d = 0; d < DIM; ++d)
            {
                TroveArray tArray;
                tArray = trove::load_array_warp_contiguous<TroveArray::size>(d_inputSamples, threadIdx.x);
                trove::make_carray(tArray, samples[d]);
                d_inputSamples += stride;
            }
        }
...
class BlockTileStore
{
    ...
    struct StoreInternal<IO_BACKEND_TROVE, DUMMY>
    {
        ...
        struct CopyCols2RowsIterateCols
        {
            template <typename D, typename P>
            static __device__ __forceinline__ void impl(
                P               pIdx,
                ...
                TroveArray &    ary)
            {
                trove::get<P::value * DIM + D::value>(ary) = samples[D::value][P::value];
            }
        };

        struct CopyCols2RowsIterateRows
        {
            template <typename D>
            static __device__ __forceinline__ void impl(
                D               dIdx,
                ...
                TroveArray &    ary)
            {
                StaticFor<0, POINTS_PER_THREAD, CopyCols2RowsIterateCols>::impl(dIdx, samples, ary);
            }
        };
        ...
        {
            TroveArray tArray;
            StaticFor<0, DIM, CopyCols2RowsIterateRows>::impl(samples, tArray);
            trove::store_array_warp_contiguous(d_outputSamples, threadIdx.x, tArray);
        }
