 ...
 *
 * @par Overview
 *     When  resizing vector capacity all data are copied to new larger, continuous memory region.
 *     It's a user responsibility to detect when capacity is reached and call vector resize
 *     function.
 *
 * @tparam     BLOCK_THREADS     Number of threads within block. Used as copy engine's template
 *                               parameter for moving data when resizing.
 * @tparam     ITEMS_PER_THREAD  Number of items copied by one thread. It's a copy engine's 
 *                               template parameter used for moving data when resizing.
 ...
 *                                   template parameter used for moving data when resizing.
 * @tparam     T                     Stored data type.
 * @tparam     RESIZE_FACTORS_COUNT  Number of resize factors. Vector may be resized using few
 *                                   defined capacity values or with few defined factors of a given
 *                                   max value.
 * @tparam     ResizeFactorT         Data type of resize factors.
 * @tparam     Enable                Helper parameter for template specializations using type
 *                                   traits.
 */
template <
    int         BLOCK_THREADS,
 ...
 *
 * @par Overview
 *     Initial vector capacity is the value of the first resize factor. Subsequent resize factors
 *     are used as a subsequent capacity values. When the last resize value is reached the vector
 *     switches to default base class resize mechanizm. That is resize enlarge factor.
 *
 * @tparam     BLOCK_THREADS         Number of threads within block. Used as copy engine's template
 *                                   parameter for moving data when resizing.
 * @tparam     ITEMS_PER_THREAD      Number of items copied by one thread. It's a copy engine's
...
private:
    ...
    /// shared storage for resize factors.
    _SharedResizeFactors &  m_sharedStorage;    
 ...
 *
 * @par Overview
 *     Initial vector capacity is the product of the first resize factor and maxCapacity. 
 *     Subsequent resize factors are used to scale maxCapacity and results are used as capacity 
 *     values. When the last resize value is reached the vector switches to default base class  
