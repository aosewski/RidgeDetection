 ...
 * @brief Counts points in @p rSqr neighbourhood of @p srcP
 *
 * @note Assumes SoA data layout.
 */
template <typename T, int blockSize>
__device__ int ctaCountNeighbouringPoints(
	T const * __restrict__ const * __restrict__ points,
	int np,
	T const * __restrict__ const * __restrict__ srcP,
	int srcP_idx,
	int dim,
	T rSqr,
	int threshold)
{
	...
	shmem[threadIdx.x] = (threadIdx.x < dim) ? srcP[threadIdx.x][srcP_idx] : 0;
	...
	for (int i = 0; i < k; ++i, x += blockSize) {
		if (x < np) {
			if (threadSqrEuclideanDistance(points, x, shmem, dim) <= rSqr) {
				(void) atomicSub(&sThreshold_, 1);
			}
		}
		__syncthreads();
