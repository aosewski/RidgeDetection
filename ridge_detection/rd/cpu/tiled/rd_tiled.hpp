...
/**
 * @brief      Structure holding all necessary data to perform ridge detection.
 *
...
struct rdTiledData
{
    /// points dimension
    size_t dim;
    /// cardinality of points set
    size_t np;
    /// cardinality of initial choosen points set
    size_t ns;
    /// max number of points inside single tile
    size_t maxTileCapacity;
    /// initial tiles count for respective dimension
    std::vector<size_t> nTilesPerDim;
    /// Algorithm parameter. Radius used for choosing points and in evolve phase.
    T r1;
    /// Algorithm parameter. Radius used for decimation phase.
    T r2;
    /// generated points parameters (s - standard deviation)
    T a, b, s;
    /// table containing points
    T const * P;
    /// table containing choosen points
    std::vector<T> S;
...
/**
 * @brief      Defines work scheme for ridge detection algorithm.
 */
enum TiledRDAlgorithm
{
    /**
     * Use local tiles in combination with mixed ridge detection scheme. Mixed ridge detection is
     * comprised of local evolution within each tile individually and global decimation phase, 
     * performed on chosen points from all tiles at the same time.
     */
    TILED_LOCAL_TREE_MIXED_RD,
    /**
     * Use grouped tiles (with list of its neighbours) in combination with mixed ridge detection
     * scheme.
     */
    TILED_GROUPED_TREE_MIXED_RD,
    /**
     * Use local tiles in combination with local ridge detection scheme performed in each tile 
     * independently.
     */
    TILED_LOCAL_TREE_LOCAL_RD,
    /**
     * Use grouped tiles (with list of its neighbours) in combination with local ridge detection
     * scheme.
     */
    TILED_GROUPED_TREE_LOCAL_RD,
    /**
     * Use extended tiles (with vector of its nearby lying points) in combination with mixed ridge
     * detection scheme.
     */
...
/**
 * @brief      CPU version of tiled ridge detection algorithm.
 *
 ...
 *
 * @param      dataPack            Structure containing all necessary data to perform ridge
 *                                 detection
 * @param[in]  cpuThreads          Number of cpu threads to use when OpenMP is enabled.
 * @param[in]  endPhaseRefinement  Whether or not to perform end phase results refinement.
 * @param[in]  verbose             Turns on drawing graphs and verbose logs.
 * @param[in]  drawTiles           Whether or not to draw each tree's tile.
 *
 * @tparam     ALGORITHM           Defines work scheme to use.
 * @tparam     T                   Point coordinates data type
 *
 * @return     Tuple containing time measurements (whole time, build tree time, 
 *             ridge detection time and refinement time)
 */
template <
