/**
 * @file test_tile_load_store_v3.cu
...
static const std::string LOG_FILE_NAME_SUFFIX = "_io-timings_v3.txt";
...
class AgentTileProcessing
{
    ...
    SampleT const * d_nativeInputSamples;
    /// Native pointer for output samples 
    SampleT * d_nativeOutputSamples;
    ...
    __device__ __forceinline__ void consumeTile(
        OffsetT                     blockOffset,
        OffsetT                     stride,
        cub::Int2Type<rd::ROW_MAJOR>    memLayout,
        cub::Int2Type<rd::ROW_MAJOR>    privateMemLayout)
    {
        typename BlockTileLoadT::ThreadPrivatePoints<rd::ROW_MAJOR> samples;
        BlockTileLoadT::loadTile2Row(d_nativeInputSamples + blockOffset * DIM, samples.data, stride);
        BlockTileStoreT::storeTile2Row(d_nativeOutputSamples + blockOffset * DIM, samples.data, stride);
    }
        ...
        cub::Int2Type<rd::COL_MAJOR>    privateMemLayout)
    {
        ...
        BlockTileLoadT::loadTile2Col(d_nativeInputSamples + blockOffset * DIM, samples.data, stride);
        BlockTileStoreT::storeTile2Col(d_nativeOutputSamples + blockOffset * DIM, samples.data, stride);
    }
    ...
    __device__ __forceinline__ void consumeTile(
        OffsetT                     blockOffset,
        OffsetT                     stride,
        cub::Int2Type<rd::COL_MAJOR>    memLayout,
        cub::Int2Type<rd::COL_MAJOR>    privateMemLayout)
    {
        typename BlockTileLoadT::ThreadPrivatePoints<rd::COL_MAJOR> samples;
        BlockTileLoadT::loadTile2Col(d_nativeInputSamples + blockOffset, samples.data, stride);
        BlockTileStoreT::storeTile2Col(d_nativeOutputSamples + blockOffset, samples.data, stride);
    ...
    {
        typename BlockTileLoadT::ThreadPrivatePoints<rd::ROW_MAJOR> samples;
        BlockTileLoadT::loadTile2Row(d_nativeInputSamples + blockOffset, samples.data, stride);
        BlockTileStoreT::storeTile2Row(d_nativeOutputSamples + blockOffset, samples.data, stride);
    }
    ...
    {
        typename BlockTileLoadT::ThreadPrivatePoints<rd::ROW_MAJOR> samples;
        BlockTileLoadT::loadTile2Row(d_nativeInputSamples + blockOffset * DIM, validPoints, samples.data, stride);
        BlockTileStoreT::storeTile2Row(d_nativeOutputSamples + blockOffset * DIM, validPoints, samples.data, stride);
    }
    __device__ __forceinline__ void consumeTile(
        OffsetT                     blockOffset,
        int                         validPoints,
    ...
    {
        typename BlockTileLoadT::ThreadPrivatePoints<rd::COL_MAJOR> samples;
        BlockTileLoadT::loadTile2Col(d_nativeInputSamples + blockOffset * DIM, validPoints, samples.data, stride);
        BlockTileStoreT::storeTile2Col(d_nativeOutputSamples + blockOffset * DIM, validPoints, samples.data, stride);
    }
    ...
    {
        typename BlockTileLoadT::ThreadPrivatePoints<rd::COL_MAJOR> samples;
        BlockTileLoadT::loadTile2Col(d_nativeInputSamples + blockOffset, validPoints, samples.data, stride);
        BlockTileStoreT::storeTile2Col(d_nativeOutputSamples + blockOffset, validPoints, samples.data, stride);
    }
    ...
    {
        typename BlockTileLoadT::ThreadPrivatePoints<rd::ROW_MAJOR> samples;
        BlockTileLoadT::loadTile2Row(d_nativeInputSamples + blockOffset, validPoints, samples.data, stride);
        BlockTileStoreT::storeTile2Row(d_nativeOutputSamples + blockOffset, validPoints, samples.data, stride);
    }
        ...
        OffsetT stride)
    {
        ...
        for (int t = blockIdx.x; t < numTiles; t += gridDim.x)
        {
            ...
            if (tileOffset + TILE_POINTS > numPoints)
            {
                ...
                consumeTile(tileOffset, numPoints - tileOffset, stride, cub::Int2Type<MEM_LAYOUT>(), cub::Int2Type<PRIVATE_MEM_LAYOUT>());
            }
            else
            {
                // Consume full tile
                consumeTile(tileOffset, stride, cub::Int2Type<MEM_LAYOUT>(), cub::Int2Type<PRIVATE_MEM_LAYOUT>());
            }
        }
    }  
...
public:
    ...
    __device__ __forceinline__ AgentTileProcessing(
        T const *               d_InputSamples,
        T *                     d_OutputSamples)
    :
        d_nativeInputSamples(d_InputSamples),
        d_nativeOutputSamples(d_OutputSamples)
    {
    }
    ...
    {
        d_nativeInputSamples += startOffset;
    ...
    OffsetT                 stride)
{
    ...
    AgentTileProcessingT(d_in, d_out).consumeRange(offset, numPoints, stride);
}
    ...
    {           
        ...
        if (debugSynchronous)
        {
            printf("Invoking deviceLoadStoreKernel<<<%d, %d, 0, %lld>>> numPoints: %d, itemsPerThread: %d, offset %d\n",
                boundsGridSize.x, kernelConfig.blockThreads, (long long)stream, numPoints, kernelConfig.itemsPerThread, offset);
        }
...
std::string createFinalGraphDataFile()
{
    ...
    graphDataFile << typeid(T).name() << "_" << g_devName << "_graphData_v3.dat";
...
void drawFinalGraph(std::string graphDataFilePath)
{
        ...
        std::ostringstream graphName;
        graphName << typeid(T).name() << "_" << g_devName << "_bandwidths_v3.png";
        ...
        cmd << "    ''        i 0 u (offset + $0 * groupStep - 7 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left,";
        cmd << "    ''        i 1 u (offset + $0 * groupStep - 5 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left,";
        cmd << "    ''        i 2 u (offset + $0 * groupStep - 3 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left,";
        cmd << "    ''        i 3 u (offset + $0 * groupStep - 1 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left,";
        cmd << "    ''        i 4 u (offset + $0 * groupStep + 1 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left,";
        cmd << "    ''        i 5 u (offset + $0 * groupStep + 3 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left,";
        cmd << "    ''        i 6 u (offset + $0 * groupStep + 5 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left,";
        cmd << "    ''        i 7 u (offset + $0 * groupStep + 7 * colStep):($2 + 0.6):2 notitle w labels rotate by 45 left ";
