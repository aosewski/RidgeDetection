    ...
    int             devClockRate)
{
    ...
    initTree->forEachNodePreorder(
        [d_chosenPointsNum, &d_initTreeLeafs, &initTreeleafCounter](NodeT * node) {
    ...
    long long int startClock = 0, endClock = 0;
    float avgTime = 0;
    for (int i = 0; i < iterations; ++i)
    {
        ...
        *d_chosenPointsNum = initChosenPointsNum;
        ...
        endClock = clock64();
        avgTime += float(endClock - startClock) / float(devClockRate);
    }

    *avgKernelTime = avgTime / iterations;
...
template <
    ...
    typename T>
void benchmark(
    TiledTreeT_RR *     d_tree_RR,
    TiledTreeT_CR *     d_tree_CR,
    TiledTreeT_CC *     d_tree_CC,
    float *         d_avgKernelTime,
    T               sphereRadius2
    )
{
    ...
    globalDecimateKernel<TiledTreeT_CR, BLOCK_SIZE, rd::ROW_MAJOR, DIM>
        <<<1,1>>>(d_tree_CR, sphereRadius2, d_avgKernelTime, g_iterations, g_devClockRate);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
    
    logValue(std::cout, *d_avgKernelTime);
    if (g_logPerfResults)
    {
        logValue(*g_logFile, *d_avgKernelTime);
    }
    ...
    globalDecimateKernel<TiledTreeT_RR, BLOCK_SIZE, rd::ROW_MAJOR, DIM>
        <<<1,1>>>(d_tree_RR, sphereRadius2, d_avgKernelTime, g_iterations, g_devClockRate);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
    ...
    globalDecimateKernel<TiledTreeT_CC, BLOCK_SIZE, rd::COL_MAJOR, DIM>
        <<<1,1>>>(d_tree_CC, sphereRadius2, d_avgKernelTime, g_iterations, g_devClockRate);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
...
{
    float * d_avgKernelTime;
    checkCudaErrors(cudaMallocManaged(&d_avgKernelTime, sizeof(float)));

    #ifdef QUICK_TEST
        benchmark<DIM, 256>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
    #else
        benchmark<DIM,   64>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,   96>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  128>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  160>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  192>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  224>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  256>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  288>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  320>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  352>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  384>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  416>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  448>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  480>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  512>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  544>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  576>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  608>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  640>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  672>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  704>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  736>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  768>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  800>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  832>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  864>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  896>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  928>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  960>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM,  992>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
        benchmark<DIM, 1024>(d_tree_RR, d_tree_CR, d_tree_CC, d_avgKernelTime, sphereRadius2);
    #endif

    checkCudaErrors(cudaFree(d_avgKernelTime));
}
    ...
    PointCloud<T> const & pc)
{
    ...
    TiledTreeT_RR * d_tree_RR = nullptr;
    checkCudaErrors(cudaMalloc(&d_tree_RR, sizeof(TiledTreeT_RR)));

    std::cout << rd::HLINE << std::endl;
    std::cout << "<<<<< ROW_MAJOR, ROW_MAJOR >>>>>" << std::endl;
    buildTree<DIM, rd::ROW_MAJOR, rd::ROW_MAJOR>(points.data(), pointNum, sphereRadius, d_tree_RR);
    ...
    TiledTreeT_CR * d_tree_CR = nullptr;
    checkCudaErrors(cudaMalloc(&d_tree_CR, sizeof(TiledTreeT_CR)));

    std::cout << rd::HLINE << std::endl;
    std::cout << "<<<<< COL_MAJOR, ROW_MAJOR >>>>>" << std::endl;
    buildTree<DIM, rd::COL_MAJOR, rd::ROW_MAJOR>(points.data(), pointNum, sphereRadius, d_tree_CR);
    ...
    TiledTreeT_CC * d_tree_CC = nullptr;
    checkCudaErrors(cudaMalloc(&d_tree_CC, sizeof(TiledTreeT_CC)));

    std::cout << rd::HLINE << std::endl;
    std::cout << "<<<<< COL_MAJOR, COL_MAJOR >>>>>" << std::endl;
    buildTree<DIM, rd::COL_MAJOR, rd::COL_MAJOR>(points.data(), pointNum, sphereRadius, d_tree_CC);

    //-----------------------------------------------------------------
    // benchmark global decimate kernel
    //-----------------------------------------------------------------

    iterateKernelConf<DIM>(d_tree_RR, d_tree_CR, d_tree_CC, sphereRadius2);

    //-----------------------------------------------------------------
    // clean-up
    //-----------------------------------------------------------------

    deleteTiledTree<TiledTreeT_CR><<<1, 1>>>(d_tree_CR);
    checkCudaErrors(cudaGetLastError());
    deleteTiledTree<TiledTreeT_RR><<<1, 1>>>(d_tree_RR);
    checkCudaErrors(cudaGetLastError());
    deleteTiledTree<TiledTreeT_CC><<<1, 1>>>(d_tree_CC);
    checkCudaErrors(cudaGetLastError());
    checkCudaErrors(cudaDeviceSynchronize());
 
    checkCudaErrors(cudaFree(d_tree_RR));
    checkCudaErrors(cudaFree(d_tree_CR));
    checkCudaErrors(cudaFree(d_tree_CC));
}
...
int main(int argc, char const **argv)
{
    ...
    g_devClockRate = devProp.clockRate;
        ...
        PointCloud<float> && fpc = SegmentPointCloud<float>(1000.f, pointNum, 3, stddev);
