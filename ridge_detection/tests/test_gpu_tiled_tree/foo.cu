 ...
 *
 */
__host__ __device__ __forceinline__ cudaError_t foo(float val)
{
    cudaError_t error = cudaSuccess;
    float val2 = val + (threadIdx.x << 2) * 0.3;
    float * ptr = nullptr;
    rdDebug(error = cudaMalloc(&ptr, sizeof(float)));
    *ptr = val2;
    printf("bid: %d tid: %d, val: %f, val2: %f\n",
        blockIdx.x, threadIdx.x, val, val2);
    rdDebug(error = cudaFree(ptr));
    // printf("val: %f, val2: %f\n",
    //     val, val2);
    return error;
}
    ...
    int                         depth)
{
    if (depth == 3)
    {
        ...
        devCheckCall(foo(in[pointNum * tileId + threadIdx.x]));
        return;
    }
