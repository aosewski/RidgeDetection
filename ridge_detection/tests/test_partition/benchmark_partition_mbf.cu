/**
 * @file benchmark_partition_mbf.cu
...
/*
 * XXX: planowałem tutaj zrobić test na rzeczywistych danych, tzn wydajność przy podziale
 * np spirali na 3x3 kafli
 */
          ...
          rd::RDSpiralParams<T> &rds)
{
    ...
    rd::gpu::BoundingBox<DIM, T> *d_bboxes, *h_bboxes;
    T *h_P;

    checkCudaErrors(cudaMalloc(&d_bboxes, g_tilesCount * sizeof(rd::gpu::BoundingBox<DIM, T>)));

    h_bboxes = new rd::gpu::BoundingBox<DIM, T>[g_tilesCount];

    // initialize bboxes

    rd::BoundingBox<T> globalBbox(h_P, rdp.np, DIM);
    globalBbox.calcDistances();
    // globalBbox.print();

    // divide point cloud on quarters
    rd::gpu::BoundingBox<DIM, T> quarterBBox[1 << DIM];

    std::vector<int> tileCoords(DIM, -1);

    auto nextSpatialIdx = [&tileCoords]()
    {
    ...
    auto calcTileBounds = [&tileCoords, &globalBbox](rd::gpu::BoundingBox<DIM, T> &b)
    {
        for (size_t i = 0; i < DIM; ++i)
        {
            b.min(i) = globalBbox.min(i) + tileCoords[i] * (globalBbox.dist[i] / 2);
            b.max(i) = globalBbox.min(i) + (tileCoords[i] + 1) * (globalBbox.dist[i] / 2);
        }
        b.calcDistances();
    };
    ...
    {
        nextSpatialIdx();
        calcTileBounds(quarterBBox[k]);
    }

    for (int k = 0; k < g_tilesCount; ++k)
    {
        h_bboxes[k] = quarterBBox[k % (1 << DIM)];
        // h_bboxes[k].print();
    }
    checkCudaErrors(cudaMemcpy(d_bboxes, h_bboxes, g_tilesCount * sizeof(rd::gpu::BoundingBox<DIM, T>), cudaMemcpyHostToDevice));


    delete[] h_bboxes;

    checkCudaErrors(cudaFree(d_bboxes));
...
static const double MAX_RAND_VALUE      = 1e16; 
...
struct IsInside
{
    ...
    __host__ __device__ __forceinline__ IsInside(
        rd::gpu::BoundingBox<DIM, T> const & bb)
    :
        bbox(bb)
    {}
    ...
    OffsetT                                 stride)
{
    ...
    OffsetT selectedPointsCnt = BlockSelectIfT(tempStorage, d_in, d_selectedItemsPtrs[blockIdx.x], selectOp).scanRange(0, numPoints, stride);
    ...
    KernelConfig                    kernelConfig)
{
    ...
    do
    {
        ...
        partitionGridSize.x = smOccupancy * g_devSMCount * SUBSCRIPTION_FACTOR;
    ...
    int const                       h_referenceSelectedPointsCnt)
{
    ...
    int blockCount = smOccupancy * g_devSMCount * SUBSCRIPTION_FACTOR;
    ...
    dispatchKernel<BLOCK_THREADS, POINTS_PER_THREAD, LOAD_MODIFIER, IO_BACKEND, INPUT_MEM_LAYOUT, DIM, STORE_TWO_PHASE>(
        d_in, numPoints, d_selectedPointsPtrs, d_selectedPointsCnt, selectOp, stride, 1, true);
    ...
    int numItems)
{
    ...
    {
        // Initialize each item to a randomly selected value from [0..2^16]
        unsigned int value;
        h_in[i] = RandomValue(MAX_RAND_VALUE);
    }
    ...
    int             numPoints)
{
     ...
     */
    kp = prepareAndRunTest<DIM, LOAD_MODIFIER, IO_BACKEND, INPUT_MEM_LAYOUT, true, T>(numPoints);
    auto perfRes = std::make_tuple(DIM, kp);
    g_graphData.push_back(perfRes);

    /**
     * store using warp-aggregated algorithm
     */
    kp = prepareAndRunTest<DIM, LOAD_MODIFIER, IO_BACKEND, INPUT_MEM_LAYOUT, false, T>(numPoints);
...
void testIOBackend(
    int             numPoints)
{
    testLoadModifier<DIM, rd::gpu::IO_BACKEND_CUB,   INPUT_MEM_LAYOUT, T>(numPoints);
    testLoadModifier<DIM, rd::gpu::IO_BACKEND_TROVE, INPUT_MEM_LAYOUT, T>(numPoints);
}
...
void testInputMemLayout(
    int             numPoints)
{
    testIOBackend<DIM, rd::ROW_MAJOR, T>(numPoints);
    testIOBackend<DIM, rd::COL_MAJOR, T>(numPoints);
    ...
    int             numPoints)
{
    ...
    #if defined(RD_DEBUG) || defined(RD_PROFILE) 
        std::cout << "\nTest DIM 2...\n\n";
        testInputMemLayout<2, T>(numPoints);
    #else
        std::cout << "\nTest DIM 1...\n\n";
        testInputMemLayout<1, T>(numPoints);
        std::cout << "\nTest DIM 2...\n\n";
        testInputMemLayout<2, T>(numPoints);
        std::cout << "\nTest DIM 3...\n\n";
        testInputMemLayout<3, T>(numPoints);
        std::cout << "\nTest DIM 4...\n\n";
        testInputMemLayout<4, T>(numPoints);
        std::cout << "\nTest DIM 5...\n\n";
        testInputMemLayout<5, T>(numPoints);
        std::cout << "\nTest DIM 6...\n\n";
        testInputMemLayout<6, T>(numPoints);
    #endif
...
int main(int argc, char const **argv)
{
    ...
    prepareAndRunTest<2, cub::LOAD_LDG, rd::gpu::IO_BACKEND_CUB, rd::ROW_MAJOR, true, float>(numPoints);
    std::cout << rd::HLINE << std::endl;
    std::cout << "FLOAT 3D: " << std::endl;
    prepareAndRunTest<3, cub::LOAD_LDG, rd::gpu::IO_BACKEND_CUB, rd::ROW_MAJOR, true, float>(numPoints);
    std::cout << rd::HLINE << std::endl;
